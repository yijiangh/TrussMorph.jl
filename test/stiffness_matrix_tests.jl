...
weight_fn = tm.get_weight_calculation_fn(t.X, r, t.T, F_m, perm, n_dof_free, node_dof, full_node_dof, t.mp, collect(1:n_v*2))
...
for i=1:iter
    ...
    # @show X
    weight = weight_fn(reshape(X, 2*n_v))
    @show weight
end
println("avg time: ", (time() - st_t) / iter)

# scene = Scene()
# tm.draw_truss!(scene, t, r, supp_scale=0.2)
